...
inline void Unencrypted(std::span<const bool> value,
...
inline void Encrypt(std::span<const bool> value,
...
class FheArray {
 public:
  ...
  static FheArray<ValueType> Unencrypted(
      std::span<const ValueType> plaintext,
      const TFheGateBootstrappingCloudKeySet* key) {
    FheArray<ValueType> shared_value(plaintext.size(), key->params);
  ...
  static FheArray<ValueType> Encrypt(
      std::span<const ValueType> plaintext,
      const TFheGateBootstrappingSecretKeySet* key) {
    FheArray<ValueType> private_value(plaintext.size(), key->params);
    private_value.SetEncrypted(plaintext, key);
    return private_value;
  }

  void SetUnencrypted(std::span<const ValueType> plaintext,
                      const TFheGateBootstrappingCloudKeySet* key) {
    assert(plaintext.size() == length_);
    ::Unencrypted(EncodedArray<ValueType>(plaintext), key, array_.get());
  }

  void SetEncrypted(std::span<const ValueType> plaintext,
                    const TFheGateBootstrappingSecretKeySet* key) {
    assert(plaintext.size() == length_);
    ::Encrypt(EncodedArray<ValueType>(plaintext), key, array_.get());
  }

  std::vector<ValueType> Decrypt(
...
class FheBasicString : public FheArray<CharT> {
 public:
  ...
  static FheBasicString<CharT> Unencrypted(
      std::span<const CharT> plaintext,
      const TFheGateBootstrappingCloudKeySet* key) {
    FheBasicString<CharT> shared_value(plaintext.size(), key->params);
    shared_value.SetUnencrypted(plaintext, key);
    return shared_value;
  }
  ...
  static FheBasicString<CharT> Encrypt(
      std::span<const CharT> plaintext,
      const TFheGateBootstrappingSecretKeySet* key) {
    FheBasicString<CharT> private_value(plaintext.size(), key->params);
    private_value.SetEncrypted(plaintext, key);
    return private_value;
  }
  ...
  std::basic_string<CharT> Decrypt(
      const TFheGateBootstrappingSecretKeySet* key) const {
    const std::vector<CharT> array = FheArray<CharT>::Decrypt(key);
